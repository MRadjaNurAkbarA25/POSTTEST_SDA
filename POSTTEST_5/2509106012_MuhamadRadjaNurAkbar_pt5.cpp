 ...
 */
void tampilkanStokSembako(BarangNode* head) { //Fungsi menampilkan stok sembako, head node pertama di linked list
    // --- LENGKAPI KODE DI SINI ---
    if (head == nullptr) {
        cout << "Gudang kosong.\n";
        return;
    } // Beri kondisi jika gudang kosong, yaitu head adalah nullptr
    BarangNode* temp = head; // Buat pointer sementara untuk melakukan transversal, dimulai dari head
    do {
        cout << "- " << temp->namaBarang << ": " << temp->stok << "\n"; // do mencetak setiap node, yaitu nama dan stok
        temp = temp->next; // pindah ke node selanjutnya menggunakan pointer ke next
    } while (temp != head); // Terus cetak sampai temp kembali ke head, jika ia menunjuk head, barang terakhir tidak akan muncul
    // ----------------------------- Jika do-while maka harus !=
}
