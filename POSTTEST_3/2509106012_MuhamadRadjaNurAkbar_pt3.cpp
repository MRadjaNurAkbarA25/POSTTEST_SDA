...
bool isFullQueue() {
    return queueRear == MAX -1;
}
bool isEmptyQueue() {
    return queueFront == -1 || queueFront > queueRear;
}
...
void stackPush(Tiket tiket) {
    if (isFullStack()) {
        cout << "Riwayat transaksi penuh!\n";
        return;
    } else {
        stackTop++;
        stackData[stackTop] = tiket;
        cout << tiket.namaPenumpang << " dicatat ke riwayat\n";
    }
}
...
void enqueue(Tiket tiket) {
    ...
    queueRear++;
    queueData[queueRear] = tiket;
    cout << tiket.namaPenumpang << " masuk antrian\n";
}
...
void dequeue() {
    ...
    Tiket tiketProses = queueData[queueFront];
    queueFront++;

    if (isEmptyQueue()) {
        queueFront = queueRear = -1;
    }
    cout << tiketProses.namaPenumpang << " berhasil diproses\n";
    cout << "Rute: "<< tiketProses.kereta.asal << "->" << tiketProses.kereta.tujuan << "\n";
    stackPush(tiketProses);
}
...
void stackPop() {
    if (isEmptyStack()) {
        ...
        Tiket riwayatSelesai = stackData[stackTop];
        stackTop--;
        cout << "Riwayat tiket " << riwayatSelesai.namaPenumpang << " dihapus\n";
    }
}
...
void peek() {
    ...
    if (isEmptyQueue()) {
        ...
        cout << "Antrian terdepan: " << queueData[queueFront].namaPenumpang << 
        "-" << queueData[queueFront].kereta.namaKereta << " (" <<
        queueData[queueFront].kereta.asal << "->" << queueData[queueFront].kereta.tujuan << ")\n";
    }
    ...
    if (isEmptyStack()) {
        ...
        cout << "Transaksi terakhir: " << stackData[stackTop].namaPenumpang <<
        "-" << stackData[stackTop].kereta.namaKereta << " (" <<
        stackData[stackTop].kereta.asal << "->" << stackData[stackTop].kereta.tujuan << ")\n";
    }
}

void displayQueue(Tiket* data, int front, int rear) {
...
void displayStack(Tiket* data, int top) {
    if (top < 0) {
        cout << "Riwayat transaksi kosong!\n";
        return;
    }
...
int main() {
    while (true) {
        ...
        switch(pilihan) {
            ...
            case 11: {
                displayQueue(queueData, queueFront, queueRear);
                cout << "Tekan Enter untuk melanjutkan...";
                cin.get();
                system("cls || clear");
                break;
            ...
            case 12: {
                displayStack(stackData, stackTop);
                cout << "Tekan Enter untuk melanjutkan...";
                cin.get();
                system("cls || clear");
                break;
